...
extern adc_err_t adc_open(adc_mode_t const       mode,
                          adc_cfg_t * const      p_cfg,
                          void         (* const  p_callback)(void *p_args));
extern adc_err_t adc_control(adc_cmd_t const     cmd,
                             void * const        p_args);

/* In RAM */
adc_ctrl_t g_dcb = { ADC_MODE_MAX, false, NULL};

/* In ROM */
extern volatile __evenaccess uint16_t * const  dreg_ptrs[];
...
//adc_err_t err=ADC_SUCCESS;
    ...
    return adc_open(mode, p_cfg, p_callback);
}
                     ...
                     uint16_t * const   p_data)
{
    ...
    if ((p_data == NULL) || (p_data == FIT_NO_PTR))
    {
        return ADC_ERR_MISSING_PTR;
    }
    if ((reg_id >= ADC_REG_MAX)
     || (dreg_ptrs[reg_id] == NULL))
    {
...
{
#if ADC_CFG_PARAM_CHECKING_ENABLE == 1
    if ((p_all_data == NULL) || (p_all_data == FIT_NO_PTR))
    {
        return ADC_ERR_MISSING_PTR;
    }
#endif
