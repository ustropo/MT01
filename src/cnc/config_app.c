...
//	{ "sys","spi", _fipn, 0, xio_print_spi,get_ui8,   xio_set_spi,(float *)&xio.spi_state,			0 },
	...
	// RXMOD{ "sys","baud",_fn,   0, cfg_print_baud,get_ui8,   set_baud,   (float *)&cfg.usb_baud_rate,		XIO_BAUD_115200 },
	// RXMOD{ "sys","net", _fipn, 0, cfg_print_net, get_ui8,   set_ui8,    (float *)&cs.network_mode,		NETWORK_MODE },
...
//RXMOD		(void)xio_ctrl(XIO_DEV_USB, yes);
	} else {
//RXMOD			(void)xio_ctrl(XIO_DEV_USB, no);
	}
	return (STAT_OK);
}
...
stat_t set_baud_callback(void)
{
	...
	//RXMOD		xio_set_baud(XIO_DEV_USB, cfg.usb_baud_rate);
	return (STAT_OK);
}
