...
*                by reading 0xFF. The RX62N cannot do this though and you have to use the blank check command.
* Arguments    : sector - 
*                    Sector to look up
*                block - 
...
uint8_t vee_blank_check_address(uint8_t *addr)
{
    ...
    /* Verify that address is on 8 byte boundary */
    ...
    /* Perform 8 byte blank check */
    ret = R_FlashDataAreaBlankCheck( (uint32_t)addr, BLANK_CHECK_8_BYTE);
...
{
    /* On the RX62N we have 8 byte boundaries */
    
    /* Check to see if it is already on an 8 byte boundary */
    if( (address & 0x07) != 0 )
    {
        /* Address is not on a boundary */
        return (address+8) & 0xFFFFFFF8;
    }
    else
    {
